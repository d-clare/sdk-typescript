...
export class AgentMemoryStoragePolicyDefinition extends Hydrator<AgentMemoryStoragePolicyDefinition> {
  constructor(model?: Partial<AgentMemoryStoragePolicyDefinition>) {
    super(model);
    if (model) {
      if (model.synthetization)
        this.synthetization = new AgentMemorySynthetizationStrategyDefinition(model.synthetization);
      if (model.metadata) this.metadata = new AgentMemoryEntryMetadataDefinition(model.metadata);
    }
  }

  /**
   * The optional template to apply per memory entry (e.g., "- {{ content }}").
   */
  template?: string;
  ...
  /**
   * The definition of the strategy, if any, to use to synthesize the agent's output before storing it into memory
   */
  @Type(() => AgentMemorySynthetizationStrategyDefinition)
  synthetization?: AgentMemorySynthetizationStrategyDefinition;

  /**
   * The definition of the strategy, if any, used to synthesize the agent's output before storing it into memory
   */
