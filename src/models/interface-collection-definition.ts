...
export class InterfaceCollectionDefinition extends Hydrator<InterfaceCollectionDefinition> {
  ...
  constructor(model?: Partial<InterfaceCollectionDefinition>) {
    super(model);
    if (model) {
      this.agents = model.agents ? Object.entries(model.agents).reduce((acc, [key, m]) => { acc[key] = new AgentInterfaceDefinition(m); return acc; }, {} as Record<string, AgentInterfaceDefinition>) : {};
      this.processes = model.processes ? Object.entries(model.processes).reduce((acc, [key, m]) => { acc[key] = new ProcessInterfaceDefinition(m); return acc; }, {} as Record<string, ProcessInterfaceDefinition>) : {};
    }
  }
